...
func FiredState(expr string, state lifecycle.ClassifierReader) (string, bool, error) {
	if expr == "" || state.IsZero() {
		return "", false, nil
	}

	t, err := template.New("").Funcs(sprigFuncMap).Option("missingkey=error").Parse(expr)
	if err != nil {
