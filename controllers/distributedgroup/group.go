...
func generateName(group *v1alpha1.DistributedGroup, i int) v1alpha1.NamespacedName {
...
func (r *Reconciler) create(ctx context.Context, group *v1alpha1.DistributedGroup, serviceList v1alpha1.ServiceSpecList) error {
	if group.Spec.Schedule == nil { // create now
		for _, spec := range serviceList {
			if err := r.createService(ctx, group, spec); err != nil {
				return errors.Wrapf(err, "cannot create service %s", spec.NamespacedName)
			}
		}
	} else { // scheduled creation
		schedule := group.Spec.Schedule

		for spec := range common.YieldByTime(ctx, schedule.Cron, serviceList...) {
			if err := r.createService(ctx, group, spec); err != nil {
				return errors.Wrapf(err, "cannot create service %s", spec.NamespacedName)
			}
		}
